...
import { mergeJson, TTL } from '../../utils/redisHelpers';
	...
	run({ client, redisClient }) {
		const runCache = async () => {
			try {
				...
				await Promise.all(instances.map(async (instance) => await mergeJson(redisClient, RedisKeys.instance(instance.InstanceID), instance, '.', TTL(15, 'Seconds'))));
			} catch (error) {
